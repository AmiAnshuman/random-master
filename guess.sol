...
contract GuessContract {
    ...
    bool isRandomCalled = false;
    
    /* stoch contract address */    
    BEP20Interface public stochContractAddress = BEP20Interface(0x089DF40b4765217dadCb788D96D23C2b068e5Def);
...
//////////////////////////////////////////////////////////////////////////////////////Modifier Definitations////////////////////////////////////////////////////////////////////////////////////////////
    ...
    modifier onlyAdmin {
        require (msg.sender ==  0xaEb4e225dA596942375f388778FAfd71DC74459D  , 'Only Admin has right to execute this function');
        _;
...
//////////////////////////////////////////////////////////////////////////////////////Staking Function//////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    */
    function chooseNumbers(uint[] memory _number) public onlyStaked() returns(uint[] memory){
        require(isRandomCalled == false, "random number is generated, can't choose number now");
    ...
    */
    ...
    function unstakeTokens() public onlyStaked() {
        ...
        require(balance < 10000, "Please call staggered Unstake function");
    ...
    // New Unstake function starts here //
    
    function staggeredUnstake(uint[] memory _number) public onlyStaked() {
        uint _amount = (_number.length).mul(100);
        uint balance = StakerInfos[msg.sender].stakingBalance;
        require(balance > 0, "staking balance cannot be 0 or you cannot stake before pool expiration period");
        require(_number.length < 100, "can not unstake all the amount");
        require(BEP20Interface(stochContractAddress).transferGuess(msg.sender, _amount.mul(10**18)));
        totalTokenStakedInContract = totalTokenStakedInContract.sub(_amount);
        
        for(uint i=0;i<StakerInfos[msg.sender].choosedNumbers.length;i++) {
            
           for(uint j=0; j<_number.length; j++){
               
                if(StakerInfos[msg.sender].choosedNumbers[i] == _number[j]){
                    
                    numerMap[StakerInfos[msg.sender].choosedNumbers[i]].isChoosen = false;
                    numerMap[StakerInfos[msg.sender].choosedNumbers[i]].userAddress = address(0);
                    
                    StakerInfos[msg.sender].currentNumbers = StakerInfos[msg.sender].currentNumbers.sub(1);
                    
                    StakerInfos[msg.sender].choosedNumbers[i] = StakerInfos[msg.sender].choosedNumbers[StakerInfos[msg.sender].choosedNumbers.length - 1];
                    delete StakerInfos[msg.sender].choosedNumbers[StakerInfos[msg.sender].choosedNumbers.length - 1];
                    StakerInfos[msg.sender].choosedNumbers.pop();
            }
           }
        }
        
        StakerInfos[msg.sender].stakingBalance = StakerInfos[msg.sender].stakingBalance.sub(_amount);
        StakerInfos[msg.sender].maxNumberUserCanChoose = StakerInfos[msg.sender].maxNumberUserCanChoose.sub(_number.length);
        
    }
    
    
    // New unstake function ends here //
