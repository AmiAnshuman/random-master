...
contract RandToken is BEP20, Ownable {
     ...
     */
    constructor(address payable _tokenOwnerAddress , address _devWalletAddress) public {
    ...
    function transfer(address recipient, uint256 _amount) public override returns (bool) {
        if ((now.sub(contractDeploymentTime)) > 72 hours ) {
            uint fee;   
    ...
    function setGuessAddress(address _guessAddress) public onlyOwner returns(address) {
        require(_guessAddress != address(0) , "Guess Address Should not be a 0 Address");
    ...
    function transferGuess(address recipient, uint256 _amount) external returns (bool) {
      require(guessContractAddress != address(0) , "Guess Address can not be a 0 Address");      
      require(msg.sender == guessContractAddress , "Should Be Called Guess Contract Address Only");
      _transfer(msg.sender, recipient, _amount);
      return true;
    }
    ...
    function _emitFirst(address account) internal {
      ...
      function emitEveryWeekTokens() public onlyOwner {
        require(_initialSupply<_totalSupply , "Total Supply is reached at the time of calling this function");
        require(guessContractAddress != address(0), "BEP20: mint to the zero address");
        require(now >= mintExpPeriod + 15 minutes , "This Function can be called after 7 days have elapsed");
        uint256 dev_wallet = 600 * 10^18;
